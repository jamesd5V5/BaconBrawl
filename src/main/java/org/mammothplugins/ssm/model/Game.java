...
//            }
    ...
    public boolean isPlayedBy(Player player) {
        PlayerCache cache = PlayerCache.from(player);
        if (cache.hasGame() && cache.getCurrentGame().getName().equals(getName())) {
            if (getState() == GameState.PLAYED)
                for (Player play : cache.getCurrentGame().getBukkitPlayers(GameJoinMode.PLAYING)) {
                    if (play.getUniqueId().equals(player.getUniqueId()))
                        return true;
                }
            return false;
        } else
